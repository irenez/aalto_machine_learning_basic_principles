...
mle_p = ones(2,22);     % initialize p_ij MLE
w = ones(1,22);     % initialize w parameter

mle_pr = sum(r_train)/N ;
for j = 1:22
    mle_p(1,j) = (a + sum(r_train(idx_0)' * x_train(idx_0,j))) / (K*a + length(r_train(idx_0)));
    mle_p(2,j) = (a + sum(r_train(idx_1)' * x_train(idx_1,j))) / (K*a + sum(r_train(idx_1)));     
